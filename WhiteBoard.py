...
cv2.waitKey(0)
cv2.imwrite('./sign_img.png', sign_img)

# Evaluating light distribution

# LDM(Px(f+k-1)) = M(Px(f-1))+[(M(Px(l+1)-M(Px(f-1)))/m]k

# marked_img= cv2.imread("sign_img.png", cv2.COLOR_BGR2GRAY)
in_img= cv2.imread("sign_img.png", cv2.IMREAD_COLOR)
marked_img= cv2.cvtColor(in_img, cv2.COLOR_BGR2GRAY)

contours, _2 = cv2.findContours(marked_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
